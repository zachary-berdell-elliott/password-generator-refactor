...
function passwordAppend(_stringEntry, _valueArray) {
  if(confirm('Would you like to include ' + _stringEntry + '?')){
    passwordIncluded.push(_valueArray);
  }
}
...
function generatePassword() {
  ...
  for(var i = passwordLength; i === 0; i--){
    passwordString += passwordIncluded[Math.floor(Math.random(0, passwordIncluded.length-1))];
  }
  console.log(passwordIncluded[Math.floor(Math.random(0, passwordIncluded.length-1))]);
